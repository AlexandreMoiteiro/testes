...
# ===============================================================
st.set_page_config(page_title="NAVLOG IFR/VFR", page_icon="🧭", layout="wide", initial_sidebar_state="expanded")
...
def build_legs(nodes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i in range(len(nodes) - 1):
        ...
        cps = []
        cp_every = int(st.session_state.ck_default)
        if cp_every > 0 and gs > 0 and ete > 0:
            k = 1
            while k * cp_every * 60 <= ete:
                t = k * cp_every * 60
                cps.append({"min": k * cp_every, "t": t, "nm": rd(gs * t / 3600.0)})
                k += 1
...
def build_pdf_payload(legs: List[Dict[str, Any]], header: Dict[str, str], start: int = 0, count: int = 22) -> Dict[str, Any]:
    ...
    d: Dict[str, Any] = {
        ...
        "MAG_VAR": f"{abs(float(st.session_state.mag_var)):.0f}°{'E' if st.session_state.mag_is_east else 'W'}",
...
def render_route_map(wps: List[Dict[str, Any]], nodes: List[Dict[str, Any]], legs: List[Dict[str, Any]], *, key: str = "mainmap") -> Dict[str, Any]:
    ...
    for L in legs:
        ...
        if st.session_state.show_cp_ticks:
            for cp in L.get("cps", []):
                latm, lonm = point_along_gc(L["A"]["lat"], L["A"]["lon"], L["B"]["lat"], L["B"]["lon"], min(float(L["Dist"]), float(cp["nm"])))
                llat, llon = dest_point(latm, lonm, L["TC"] - 90, 0.35)
                rlat, rlon = dest_point(latm, lonm, L["TC"] + 90, 0.35)
                folium.PolyLine([(llat, llon), (rlat, rlon)], color="#111827", weight=2, opacity=1).add_to(m)

        if st.session_state.show_doghouses and L["Dist"] > 0.4:
            mid = point_along_gc(L["A"]["lat"], L["A"]["lon"], L["B"]["lat"], L["B"]["lon"], L["Dist"] / 2)
            side = 1 if (L["i"] % 2 == 0) else -1
            anchor = dest_point(mid[0], mid[1], L["TC"] + side * 90, 0.9)
            folium.PolyLine([mid, anchor], color="#111827", weight=1, opacity=0.8).add_to(m)
            html = f"""
            <div style="transform:translate(-50%,-50%);background:white;border:1px solid #0f172a;border-radius:10px;padding:4px 6px;box-shadow:0 1px 4px rgba(0,0,0,.25);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;white-space:nowrap;">
              <b>{deg3(L['MH'])}</b> / {deg3(L['TC'])}<br>{int(round(L['A']['alt']))} ft<br>{pdf_time(L['time_sec'])}
            </div>"""
            add_div_marker(m, anchor[0], anchor[1], html)
...
ss("selected_airways", [])
ss("show_cp_ticks", True)
ss("show_doghouses", True)
ss("saved_routes", {})
ensure_point_ids()
...
with st.sidebar:
    ...
    ac = st.selectbox("Tipo", ac_names, index=ac_names.index(st.session_state.aircraft_type) if st.session_state.aircraft_type in ac_names else 0)
    ...
    st.number_input("Checkpoints cada min", 1, 15, key="ck_default")
    st.number_input("Altitude default p/ novos pontos", 0.0, 45000.0, key="default_alt", step=100.0)
...
# TABS
# ===============================================================
tab_route, tab_map, tab_navlog, tab_data = st.tabs(["🧩 Rota", "🗺️ Mapa", "📄 Navlog / PDF", "🗃️ Dados"])
...
with tab_map:
    ...
    c1, c2 = st.columns([1, 1])
    with c1:
        st.toggle("Checkpoints", key="show_cp_ticks")
    with c2:
        st.toggle("Doghouses", key="show_doghouses")
...
with tab_navlog:
    if not st.session_state.legs:
        ...
        with c10:
            fl_alt = st.text_input("FL/Altitude header", "")
        with c11:
...
with tab_data:
    ...
    st.markdown("#### Rotas padrão no Gist")
    ...
    c1, c2 = st.columns(2)
    with c1:
        route_name = st.text_input("Guardar rota atual como", "")
    ...
    with c2:
        ...
        if choice and st.button("Carregar", use_container_width=True):
            ...
            recalc_route()
            st.rerun()
        if choice and st.button("Apagar rota guardada", use_container_width=True):

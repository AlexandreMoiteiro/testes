...
# - Carrega pontos IFR de CSV local (sem fetch à eAIP em runtime)
# - Carrega airways/segmentos de CSV local (opcional)
...
# streamlit pandas folium streamlit-folium pdfrw reportlab
# ---------------------------------------------------------------
...
IFR_POINTS_CSV = "IFR_POINTS_PORTUGAL.csv"
IFR_AIRWAYS_CSV = "IFR_AIRWAYS_LOW_PORTUGAL.csv"
...
# Designadores tipicos de ATS routes/airways (ex.: UN873, UM30, L14, N13).
# Usado para extrair rotas da coluna "routes" do CSV IFR_POINTS_PORTUGAL.csv.
ROUTE_TOKEN_RE = re.compile(r"\b(?:U?[A-Z]{1,3}\d{1,4}[A-Z]?|[A-Z]{2,5})\b")
...
# ===============================================================
# DATA LOADING: LOCAL IFR POINTS + AIRWAYS CSV
# ===============================================================

REQUIRED_IFR_POINT_COLUMNS = ["code", "lat", "lon"]

def normalize_ifr_points_df(df: pd.DataFrame) -> pd.DataFrame:
    """Normaliza o CSV local de pontos IFR. A app NÃO vai buscar dados à eAIP."""
    if df is None or df.empty:
        return pd.DataFrame(columns=["src", "code", "name", "lat", "lon", "alt", "routes", "remarks", "city", "sector"])
    df = df.copy()
    df.columns = [str(c).strip().lower() for c in df.columns]
    rename = {"ident": "code", "latitude": "lat", "longitude": "lon", "ats_routes": "routes", "route": "routes", "remark": "remarks"}
    df = df.rename(columns={k: v for k, v in rename.items() if k in df.columns})
    missing = [c for c in REQUIRED_IFR_POINT_COLUMNS if c not in df.columns]
    if missing:
        st.warning(f"{IFR_POINTS_CSV} existe mas faltam colunas: {', '.join(missing)}. Esperado pelo menos: code,lat,lon")
        return pd.DataFrame(columns=["src", "code", "name", "lat", "lon", "alt", "routes", "remarks", "city", "sector"])
    df["code"] = df["code"].astype(str).str.upper().str.strip()
    df["lat"] = pd.to_numeric(df["lat"], errors="coerce")
    df["lon"] = pd.to_numeric(df["lon"], errors="coerce")
    df = df.dropna(subset=["code", "lat", "lon"])
    for col, default in [("src", "IFR"), ("name", None), ("alt", 0.0), ("routes", ""), ("remarks", ""), ("city", ""), ("sector", "")]:
        if col not in df.columns:
            df[col] = default if default is not None else df["code"]
    df["src"] = "IFR"
    df["name"] = df["name"].fillna(df["code"]).astype(str).str.upper().str.strip()
    df["alt"] = pd.to_numeric(df["alt"], errors="coerce").fillna(0.0)
    keep = ["src", "code", "name", "lat", "lon", "alt", "routes", "remarks", "city", "sector"]
    return df[keep].drop_duplicates(subset=["code", "lat", "lon"]).sort_values("code").reset_index(drop=True)


def parse_routes_cell(cell: Any) -> List[str]:
    txt = str(cell or "").upper()
    toks = ROUTE_TOKEN_RE.findall(txt)
    return sorted(set(t for t in toks if t != "NIL"))


def load_local_ifr_airways(points: pd.DataFrame) -> Tuple[Dict[str, List[Dict[str, Any]]], pd.DataFrame]:
    """
    Lê IFR_AIRWAYS_LOW_PORTUGAL.csv se existir.
    Formato recomendado: route,seq,code,lat,lon
    Se o ficheiro não existir, cria airways aproximadas a partir da coluna 'routes' do CSV de pontos, ordenadas por posição.
    Isso permite pesquisar route designators, mas para ordem oficial de uma airway recomenda-se preencher o CSV de airways.
    """
    airways: Dict[str, List[Dict[str, Any]]] = {}
    if os.path.exists(IFR_AIRWAYS_CSV):
        raw = load_csv_safely(IFR_AIRWAYS_CSV)
        if raw is not None and not raw.empty:
            raw = raw.copy()
            raw.columns = [str(c).strip().lower() for c in raw.columns]
            raw = raw.rename(columns={"ident": "code", "name": "code", "latitude": "lat", "longitude": "lon", "order": "seq"})
            need = {"route", "seq", "code", "lat", "lon"}
            if need.issubset(raw.columns):
                raw["route"] = raw["route"].astype(str).str.upper().str.strip()
                raw["code"] = raw["code"].astype(str).str.upper().str.strip()
                ...
                raw = raw.dropna(subset=["route", "seq", "code", "lat", "lon"]).sort_values(["route", "seq"])
                for route, grp in raw.groupby("route"):
                    airways[route] = [{"route": route, "name": r["code"], "code": r["code"], "lat": float(r["lat"]), "lon": float(r["lon"])} for _, r in grp.iterrows()]
            else:
                st.warning(f"{IFR_AIRWAYS_CSV} existe mas o formato esperado é route,seq,code,lat,lon. Vou tentar inferir pela coluna routes dos pontos.")

    if not airways and not points.empty and "routes" in points.columns:
        buckets: Dict[str, List[Dict[str, Any]]] = {}
        for _, r in points.iterrows():
            for route in parse_routes_cell(r.get("routes", "")):
                buckets.setdefault(route, []).append({
                    "route": route, "name": str(r["code"]), "code": str(r["code"]), "lat": float(r["lat"]), "lon": float(r["lon"]),
                })
        # fallback: ordenação geográfica, útil para visualizar/pesquisar, mas não substitui ordem oficial.
        for route, pts in buckets.items():
            if len(pts) >= 2:
                airways[route] = sorted(pts, key=lambda p: (p["lat"], p["lon"]))
    ...
    for route, pts in airways.items():
        for i in range(len(pts) - 1):
            a, b = pts[i], pts[i + 1]
            segments.append({
                "route": route, "seq": i + 1, "from": a["name"], "to": b["name"],
    ...
    return airways, pd.DataFrame(segments)


@st.cache_data(show_spinner=False)
def load_local_ifr_data() -> Tuple[pd.DataFrame, Dict[str, List[Dict[str, Any]]], pd.DataFrame, Dict[str, str]]:
    raw = load_csv_safely(IFR_POINTS_CSV) if os.path.exists(IFR_POINTS_CSV) else pd.DataFrame()
    points = normalize_ifr_points_df(raw)
    airways, segs = load_local_ifr_airways(points)
    meta = {
        "points_source": IFR_POINTS_CSV if os.path.exists(IFR_POINTS_CSV) else "ficheiro não encontrado",
        "airways_source": IFR_AIRWAYS_CSV if os.path.exists(IFR_AIRWAYS_CSV) else "inferido da coluna routes",
    }
...
def render_route_map(db: pd.DataFrame, seg_df: pd.DataFrame):
    ...
    # Mantém o mapa focado em Portugal continental.
    mainland_bounds = [[PORTUGAL_MAINLAND_BBOX["lat_min"], PORTUGAL_MAINLAND_BBOX["lon_min"]], [PORTUGAL_MAINLAND_BBOX["lat_max"], PORTUGAL_MAINLAND_BBOX["lon_max"]]]
    folium.Rectangle(mainland_bounds, color="#0f172a", weight=1, fill=False, opacity=0.35, tooltip="Portugal continental").add_to(m)
    m.fit_bounds(mainland_bounds)
    folium.LayerControl(collapsed=False).add_to(m)
    return st_folium(m, width=None, height=720, returned_objects=["last_clicked", "bounds"], key="main_map")
...
<div class='hero'>
  <h1>✈️ {APP_TITLE}</h1>
  <p>Planeamento NAVLOG com CSV local de pontos IFR, airways/RNAV, fixes por radial/DME, VOR inbound/outbound, mapa continental e PDF.</p>
</div>
""",
    unsafe_allow_html=True,
)

# Load data: everything is local in the repository. No eAIP fetch at runtime.
with st.spinner("A carregar CSVs locais..."):
    ifr_points, airways, airway_segments, ifr_meta = load_local_ifr_data()
    if ifr_points.empty:
        st.warning(f"Não encontrei pontos IFR em {IFR_POINTS_CSV}. Coloca o CSV na raiz do repositório com colunas code,lat,lon,routes,remarks.")
...
with st.sidebar:
    ...
    st.caption(f"v{APP_VERSION} · CSV local")
    st.caption(f"IFR: {ifr_meta.get('points_source', '—')}")
    st.caption(f"Airways: {ifr_meta.get('airways_source', '—')}")
    st.markdown("---")
...
with map_tab:
    ...
    if st.session_state.wps:
        map_data = render_route_map(db, airway_segments)
    else:
        ...
        folium.Rectangle(mainland_bounds, color="#0f172a", weight=1, fill=False, opacity=0.35, tooltip="Portugal continental").add_to(m)
        m.fit_bounds(mainland_bounds)
        map_data = st_folium(m, width=None, height=620, returned_objects=["last_clicked", "bounds"], key="empty_map")

    with st.expander("➕ Adicionar ponto VFR por clique no mapa", expanded=False):
        st.caption("Clica no mapa e depois confirma aqui. Isto mantém o comportamento antigo para pontos VFR/manual.")
        clicked = (map_data or {}).get("last_clicked") if isinstance(map_data, dict) else None
        c1, c2 = st.columns([2, 1])
        click_name = c1.text_input("Nome do ponto", "WP VFR")
        click_alt = c2.number_input("Alt ft", 0.0, 45000.0, float(st.session_state.get("last_click_alt", 3000.0)), step=100.0, key="last_click_alt")
        st.write("Último clique:", clicked if clicked else "—")
        if st.button("Adicionar clique à rota", use_container_width=True) and clicked:
            add_wp(click_name, float(clicked["lat"]), float(clicked["lng"]), float(click_alt), kind="VFR")
            clear_computed()
            st.rerun()
...
with data_tab:
    ...
    st.json({
        ...
        "IFR points CSV": int(len(ifr_points)),
    ...
    with dtab1:
        ...
        st.download_button("⬇️ IFR points CSV", ifr_points.to_csv(index=False).encode("utf-8"), IFR_POINTS_CSV, use_container_width=True)
    with dtab2:
